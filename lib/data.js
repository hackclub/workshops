...
export const getWorkshopFile = async slug =>
  await getRawFileFromRepo(`/workshops/${slug}/README.md`)
...
export const getWorkshopSections = async () => {
  ...
  return (await Object.keys(manifest).map(async key => {
    let workshops = []
    await manifest[key].slugs.forEach(async slug => {
      ...
      workshops.push({ slug, name, description, img, tags, author, html })
    })
