...
export const getConductHtml = async () => {
  const md = fs.readFileSync(`./public/content/CODE_OF_CONDUCT.md`, 'utf8')
...
export const getWorkshopSections = () =>
  Object.keys(manifest).map(key => {
    ...
    manifest[key].slugs.forEach(slug => {
      ...
      let { name, description, img = null } = matter(md).data
      workshops.push({ slug, name, description, img })
    })
...
export const getWorkshopData = async (slug, md, branch) => {
  ...
  const authors = (data?.author || '').includes('@')
    ? data?.author
        .replaceAll('@', '')
        .replaceAll('ad510', '') // user has no profile picture
        .split(',')
        .map(trim)
        .map(
