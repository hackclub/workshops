...
const ghRest = async p => {
  ...
  if (!r.ok)
    throw new Error(`GitHub REST API error (${r.status}): ${await r.text()}`)
  return r.json()
}
...
const ghCommitAuthors = async (repo, path) =>
  (
    await ghRest(
      `/repos/hackclub/${repo}/commits?path=${encodeURIComponent(path)}&per_page=100`
    )
  )
    .filter(c => c.author)
...
export const getWorkshopSlugs = async () =>
  (await ghDirs('hackclub', 'workshops')).filter(
    p => !['img', 'lib', 'README.md'].includes(p)
  )
...
export const getVIPNewsletterSlugs = async () =>
  (await ghDirs('vip-newsletters')).filter(
    p => !['.gitignore', 'README.md'].includes(p)
  )
...
export const getVIPNewslettersHtml = async () =>
  markdownToHtml(
    await getRawFileFromRepo('/README.md', 'main', 'hackclub/vip-newsletters'),
    'README.md',
    '',
    true
  )
...
export const getLeaderNewsletterSlugs = async () => {
  ...
  return Promise.all(
    dirs
      .filter(n => n === 'updates' || n === 'archive')
      .map(async n => ({
        kind: n,
        slugs: (
          await ghRest(`/repos/hackclub/leaders-newsletter/contents/${n}`)
...
export const getNewsletterSlugs = async () =>
  (await ghDirs('newsletter')).filter(
    p => !['.gitignore', 'README.md'].includes(p)
  )
...
export const getNewslettersHtml = async () =>
  markdownToHtml(
    await getRawFileFromRepo('/README.md', 'main', 'hackclub/newsletter'),
    'README.md',
    '',
    true
  )
...
export const getBannerHtml = async () =>
  markdownToHtml(
    await getRawFileFromRepo('README.md', 'master', 'hackclub/banner'),
    'README.md',
    '',
    true
  )
...
const tagCreator = tagString => {
  ...
  if (tagString.length) {
    ...
    const maxLim = tags.length > 5 ? 5 : tags.length
    ...
    manifest[key].slugs.forEach(slug => {
      if (Object.keys(custom).includes(slug)) {
        ...
        try {
          ...
          console.error(
            `Skipping workshop "${slug}" due to invalid frontmatter: ${e.message}`
          )
          return
        }
        let { name, description, img = null, tags = [], author } = data
